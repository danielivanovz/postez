...
describe('End-to-End Integration Tests with Real PostgreSQL', () => {
  ...
  async function mainWithoutViews(
    ...
    const _interfaces = await parseInterfaces(db, tables, pg.default.sql('select-table-information'), generateEnumMap(enums), typesSchema, schema);
  ...
  describe('File I/O utilities', () => {
    test('should create directory and write formatted file', async () => {
      ...
      expect(fileContent).toMatch(/{\s+id: number;\s+name: string;\s+}/);
    });
  });
});
