...
export async function gen(db: IDatabase<unknown, IClient>, outputPath: string) {
  ...
  const _interfaces = await parseInterfaces(db, tables, pg.sql('select-table-information'), generateEnumMap(enums))

  try {
    writeToFile(outputPath, _enums.concat(_interfaces), 'types')
