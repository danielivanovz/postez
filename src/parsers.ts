...
export async function generateTableTypes(rootPath: string, tables: string[]) {
	...
	const tablesTypes = `export type TableTypes = ${tables.map((x) => `'${x}'`).join(' | ')}`

	try {
		if (!fs.existsSync(rootPath)) fs.mkdirSync(rootPath)

		fs.writeFileSync(schemaPath, prettier.format(tablesTypes, { ...prettierConfig, filepath: schemaPath }))
...
export async function generateInteraces(
	...
	try {
		...
		fs.writeFileSync(schemaPath, prettier.format(fileContent, { ...prettierConfig, filepath: schemaPath }))
...
export async function generateEnums(rootPath: string, enums: IEnumSchema[]): Promise<void> {
